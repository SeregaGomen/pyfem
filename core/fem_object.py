...
class TObject:
    ...
    def save_json(self, file_name):
        ...
        for i in range(0, len(self.__results__)):
            results['function'] = self.__results__[i].name
            results['t'] = self.__results__[i].t
            results['results'] = self.__results__[i].results
...
#        c_map = cm.get_cmap(name='terrain', lut=None)
    ...
    def get_surface_color(self, res):
        ...
        u_max = max(res)
        face_colors = []
        for i in range(0, len(self.__mesh__.be)):
            u = 0
            for j in range(0, len(self.__mesh__.be[i])):
                u += res[self.__mesh__.be[i][j]]
            u /= len(self.__mesh__.be[i])
