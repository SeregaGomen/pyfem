...
def quad4(res_name):
    ...
    if obj.set_mesh('mesh/quad-4.trpa'):
        ...
        obj.add_young_modulus('203200')
        obj.add_poisson_ratio('0.27')
        obj.add_boundary_condition(DIR_X | DIR_Y, '0', 'y == -0.5')
        # obj.add_temperature('100')
        # obj.add_alpha('1.25E-5')
        # obj.add_volume_load(DIR_2, '-0.05')
        # obj.add_surface_load(DIR_2, '-0.05', 'y == 0.5')
        obj.add_concentrated_load(DIR_Y, '-0.05', 'y == 0.5 and (x == -0.5 or x == 0.5)')
        if obj.calc():
            obj.print_result()
            obj.save_result(res_name)
            TPlot(res_name)
...
def beam2d3(res_name):
    ...
    if obj.set_mesh('mesh/beam2d-3.trpa'):
        ...
        obj.add_young_modulus('203200')
        obj.add_poisson_ratio('0.27')
        obj.add_boundary_condition(DIR_Y, '0', '(x == -5 and y == -0.25) or (x == 5 and y == -0.25)')
        # obj.add_boundary_condition(DIR_1 | DIR_2, '0', '(x == -5) or (x == 5)')
        obj.add_thickness('0.01')
        obj.add_surface_load(DIR_Y, '-1', 'y == 0.25')
        if obj.calc():
            obj.print_result()
            obj.save_result(res_name)
            TPlot(res_name)
...
#        obj.set_solve_method('iterative')
        ...
        obj.add_boundary_condition(DIR_X | DIR_Y | DIR_Z, '0', 'abs(z + h) <= eps')
        obj.add_boundary_condition(DIR_X, '0', 'abs(x) <= eps')
        obj.add_boundary_condition(DIR_Y, '0', 'abs(y) <= eps')

        obj.add_surface_load(DIR_X, 'p*cos(atan2(y,x))', '(z <= 0 and z >= -l)')
        obj.add_surface_load(DIR_Y, 'p*sin(atan2(y,x))', '(z <= 0 and z >= -l)')

        obj.add_surface_load(DIR_X, 'p*cos(atan2(y,x))*sin(atan2((x**2+y**2)**0.5,(z -c0)))',
                             'abs(x**2 + y**2 + (z - c0)**2 - r**2) <= eps')
        obj.add_surface_load(DIR_Y, 'p*sin(atan2(y,x))*sin(atan2((x**2+y**2)**0.5,(z - c0)))',
                             'abs(x**2 + y**2 + (z - c0)**2 - r**2) <= eps')
        obj.add_surface_load(DIR_Z, 'p*cos(atan2((x**2+y**2)**0.5,(z - c0)))',
                             'abs(x**2 + y**2 + (z - c0)**2 - r**2) <= eps')

        obj.add_surface_load(DIR_X, 'p*cos(atan2(y,x))*sin(atan2((x**2+y**2)**0.5,(z -c1)))',
                             'abs(x**2 + y**2 + (z - c1)**2 - r**2) <= eps')
        obj.add_surface_load(DIR_Y, 'p*sin(atan2(y,x))*sin(atan2((x**2+y**2)**0.5,(z - c1)))',
                             'abs(x**2 + y**2 + (z - c1)**2 - r**2) <= eps')
        obj.add_surface_load(DIR_Z, 'p*cos(atan2((x**2+y**2)**0.5,(z - c1)))',
                             'abs(x**2 + y**2 + (z - c1)**2 - r**2) <= eps')
